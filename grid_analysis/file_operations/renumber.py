...
def renumber_residues(pdb, starting_resid):
...
def main(pdbfh, stdout=None, starting_resid=None):
    ...
    try:
        ...
        for lineno, line in enumerate(new_pdb):
            if not (lineno % _buffer_size):
                stdout.write(''.join(_buffer))
                _buffer = []
            #line = line.rstrip('\n')
            _buffer.append(line)
        for line in _buffer:
            stdout.write(line)
        #_buffer = list(map(str.rstrip, _buffer))
        #stdout.write(''.join(_buffer))
        #stdout.flush()
    except IOError:
